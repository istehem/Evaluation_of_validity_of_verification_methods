...
wdgm_global_status_ok(_S) ->
  [{wdgm_global_status_ok,{call,?MODULE,transition,[]}}].

%% -Calculate transitions-------------------------------------------------------

transition() ->
  gen_fsm:send_event(wdgmfsm,transition).

%% -WdgM_Init-------------------------------------------------------------------

initwdgm_pre(_F,_T,S,_C) ->
  S#state.initialized /= true.

initwdgm_command(_S) ->
  {call, ?MODULE, initwdgm, [frequency([{20, return({eqc_c:address_of('Tst_Cfg1'), false})},
...
%  end.
%
%getmode_post(_F,_T,S, [Is_Null], {R, Mode}) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
%  case R of
...
%
%setmode_post(_F,_T,S, [ModeId, Cid], Ret) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
...
%%% [WDGM186], [WDGM142]
%
%setmode_next(_F,_T,S, Ret, [ModeId, _Cid]) ->
%  case Ret of
%    0 -> case
...
%deinit_post(_F,_T,S, _Args, _Ret) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
...
%  end.
%
%deinit_next(_F,_T,S, _Ret, _Args) ->
%    case S#state.globalstatus of
%      'WDGM_GLOBAL_STATUS_OK' ->
...
%checkpointreached_pre(_F,_T,S,_) ->
%  S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect
%    orelse
%    S#state.initialized.
%
...
%
%checkpointreached_post(_F,_T,S, Args=[SEid, CPId], Ret) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
...
%
%checkpointreached_next(_F,_T,S, _Ret, Args = [SEid, CPid]) ->
...
%  end.
%
%getlocalstatus_post(_F,_T,S, [SEid, Is_Null], {Ret, Status}) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
%  case Ret of
...
%  end.
%
%getlocalstatus_next(_F,_T,S, _Ret, _Args) ->
%  S.
%
%%% -WdgM_GetGlobalStatus--------------------------------------------------------
%
%getglobalstatus_pre(_F,_T,S,_) ->
%  S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect
%    orelse
%    S#state.initialized.
%
...
%  end.
%
%getglobalstatus_post(_F,_T,S, [Is_Null], Ret) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
%  case Ret of
...
%  end.
%
%getglobalstatus_next(_F,_T,S, _Ret, _Args) ->
%  S.
%
%%% -WdgM_PerformReset-----------------------------------------------------------
%
%performreset_pre(_F,_T,S,_) ->
%  S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect
%    orelse
%    S#state.initialized.
%
...
%
%performreset_post(_F,_T,S, _Args, _Ret) ->
%  DevErrorDetect = S#state.originalCfg#wdgm.wdgmgeneral#wdgmgeneral.dev_error_detect,
...
%
%performreset_next(_F,_T,S, _Ret, _Args) ->
%  S.
%
%%% -WdgM_GetFirstExpiredSEID----------------------------------------------------
%
%getfirstexpiredseid_pre(_F,_T,_S,_) ->
%  true. %% [WDGM348]
%
...
%  end.
%
%getfirstexpiredseid_post(_F,_T,S, [Is_Null], Ret) ->
...
%  end.
%
%getfirstexpiredseid_next(_F,_T,S, _Ret, _Args) ->
%  S.
%
%%% -WdgM_MainFunction-----------------------------------------------------------
%
%mainfunction_pre(_F,_T,S,_) ->
...
%
%mainfunction_post(_F,_T,S, _Args, _Ret) ->
...
%  NextS = mainfunction_next(global_status_ok,global_status_ok,S, 0, 0),
%  MonitorTable = eqc_c:value_of('WdgM_SupervisedEntityMonitorTable'),
%
%  Behaviour =
...
%  end.
%
%mainfunction_next(_F,_T,S, _Ret, _Args) ->
%  case
%    S#state.initialized
%  of
...
%weight(_F,_T,_S, setmode) -> 2;
%weight(_F,_T,_S, checkpointreached) -> 10;
%weight(_F,_T,_S, mainfunction) -> 50;
%weight(_F,_T,S, initwdgm) -> case S#state.initialized of
%                      true -> 0;
%                      _    -> 200
%                   end;
%weight(_F,_T,S,deinit) -> case S#state.initialized of
%                      true -> 0;
%                      _    -> 0
%                    end;
%weight(_F,_T,_S,getfirstexpiredseid_pre) -> 0;
%weight(_F,_T,_S,getmode)                 -> 0;
%weight(_F,_T,_S,getglobalstatus)         -> 0;
%weight(_F,_T,S,performreset) -> case S#state.globalstatus of
%                           'WDGM_GLOBAL_STATUS_STOPPED' -> 200;
%                           _                            -> 0
%                          end;
weight(_F,_T,_S, _Cmd) -> 1.
...
start() ->
  wdgm_eqc:start(),
  gen_fsm:start({local,wdgmfsm},?MODULE,[],[]).

stop() ->
  gen_fsm:sync_send_all_state_event(wdgmfsm,stop).

collect_res(_H,_S,_Res,Cmds) ->
  Xs = lists:filter(fun({_,_,{_,_,Name,_}}) -> Name == performreset end,Cmds),
  case Cmds of
    [] -> [0];
    Ys -> [length(Xs)/length(Ys)]
  end.
