...
e2e_protect_next(S, _, [#cfg{data_id = DataId}, ModelCounter,Data]) ->
  S#state{ sender = #sender{counter = (ModelCounter + 1) rem 15,
                            msg = model_protect(DataId, ModelCounter, Data)}}.
...
send_args(#state{cfg = #cfg{data_length=Len}}) ->
  [subset(3,lists:seq(0,8*Len-1))].
