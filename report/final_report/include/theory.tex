...
``incomplete'' if a series of tests passes but it is not complete
...
be put together as one property that should hold for the hole
...
Quviq together with Thomas Arts. Quviq offers a
...
in Erlang and checked against API calls to the C-code, this is called model
based testing. It is not necessary to have the actual source code; it is enough
to only supply the compiled byte code and some library files of the program to
be able to test it. \cite{QUVIQ:about}\cite{QUICKCHECK:manual}

QuickCheck uses property based testing, which means that system requirements are
implemented and tested as properties. QuickCheck makes also use of random
testing, but has guided random test generation. This means that the samples can
be weighted to cover certain parts of the state space with more likelihood.
