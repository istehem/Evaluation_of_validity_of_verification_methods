...
are uniformly distributed. To test an if-statement, which
compares equality of an 32-bits integer variable $v \in \mathbb{N}_{32}$
to a fixed value $v_{comp} \in \mathbb{N}_{32}$,
its needed to generate $v_{comp}$. Otherwise the if-statement will not evaluate to true.
...
and thereby drastically be able to increase the number of test cases.
...
$\mathbb{N}_{32}$\textbackslash$\{v_{comp}\}$, hence every other 32 bit integer.
...
true and the test case form $\mathbb{N}_{32}$\textbackslash$\{v_{comp}\}$ will
