...
compares equality of an 32-bits integer variable to a certain value,
one needs to generate that value. If test cases are uniformly
chosen, the likelihood of generating such a value is $P = \frac{1}{2^{32}}$.
\cite{RandomTesting:Microsoft}

However, in white box testing, code knowledge can be added to the
generation of test cases, narrowing the input domain and greatly
decreasing the number of test cases. Sub domains of the input region
can be chosen to represent both valid conditions, as well as invalid
conditions. In the example above, one sub domain can contain the
32-bit integer variable, and another could contain all $2^{32}-1$
other integers. Then only one test case is needed from each sub
domain.
...
no actual test performed on the system. Instead the model is verified.  Model
