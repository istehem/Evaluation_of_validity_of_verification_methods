...
\in V \rightarrow c_k \in S$. For the graph $G$ there exist a start checkpoint
$c_s \in V$ and a final checkpoint $c_f \in V$.  Then the logical supervision
checks that the first checkpoint $c_i$ that is reached has the property $c_i =
c_s$ and for every next coming checkpoint $c_j$ there exists an edge
$(c_j,c_{j+1}) \in E, c_j \neq c_f$.           
...
\begin{figure}
\includegraphics{pictures/localstatuses.jpg}
...
\paragraph{Bad transition}
The transition \lstinline!WDGM_GLOBAL_STATUS_DEACTIVATED! to
\lstinline!WDGM_GLOBAL_STATUS_STOPPED! is possible due to [WDGM139]

\paragraph{Incorrect reference}
[WDGM329] could be the same as [WDGM]
Initial/Final?

\paragraph{Optional or mandatory}
[WDGM344] and [WDGM258] is the same, with one difference: one is optional, the
other mandatory.

\paragraph{Logical supervision results}
AUTOSAR does not specify if it is possible to overwrite logical supervision
results from the same supervised entity.
I.e.
\begin{lstlisting}
WdgM_CheckpointReached() -> incorrect result for SEx
WdgM_CheckpointReached() -> Correct result for SEx
\end{lstlisting}

\paragraph{Wrong spelling}
[WDGM344\_CONF] WdgMInternallCheckpointFinalRef
