...
%% write what formal/semi-formal verification means according to ISO.

\section{Achieving good test cases}
%% how do we generate good test cases?
The finding good test cases are not very trivial. It may not be enough to
...
put in an absorbing state and hence only a few relevant API-calls will be generated.
Negative testing mostly only needs a short sequences of API-calls before a
single property will be tested. As a conclusion negative testing needs more test
cases, but those tests can be small. A problem using Quickcheck is that the test
cases are generated before the actual execution of the program. Hence it is very
likely that a lot of API-calls will be executed after a particular property has
been tested and the watchdog manager will be put in an absorbing state, when
using negative testing. As a consequence negative testing is time consuming.


\subsection{Tweaking the generators}
%% tweaking the generators to find interesting test cases, with functional
%% safety in mind

\subsection{Collapsing states}
%% write something that QuickCheck automatically does this

\section{Evaluation of the solution}
\subsection{Functional safety}
%% The question here that needs to be answered is:
%% Does the solution ensure functional safety?

\subsection{Measurements of the state space}
%% How does one measure the size of the state space that is actually verified?
...
The coverage results after running all configuration can be seen in table
