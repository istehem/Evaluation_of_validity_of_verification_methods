...
(integrate?) the extra functionality, each ECU will \textit{(also?)} need to
become more complex \cite{Springer:advanced_microsystems}. \textit{(F\"{o}rklara
  mer ing{\aa}ende att ECU blir mer komplex pga man typ inte kan l\"{a}gga in
  fler ECUs i bilar f\"{o}r d{\aa} \"{o}kar energianv\"{a}ndningen mer \"{a}n om
  man fixar b\"{a}ttre ECUer \cite{Springer:advanced_microsystems}.)}
...
development should be made from scratch to finish \cite{ISO26262}.
