...
requirements held. In other words that the API calls were called correctly.

\subsection{Formal Notation}
To be able to automatically generate test cases, using QuickCheck,
AUTOSAR specifications, written in a natural language, needed to be transformed
into Erlang code. Hence transforming informal notation into formal notation.

A problem when translating the AUTOSAR specifications into code was that there
were ambiguities. It was easy to see that there were room for different interpretations,
which most likely would
result in errors later.

The translating process was done iteratively as described in section \ref{SEC:ITERATIVE}

\subsection{Independence of the Erlang implementation}
The implementation in Erlang was done independent of the design
choices of the C-code. The idea was to ensure an independent model; if the model
was inspired by the C-code, it could have transmitted errors. Also implementing the
Erlang module independent of the C-implementation would result in that ambigousities in
the AUTOSAR specification would be easier to find. This since two different interpretation
of the same specification would eventually be availible.
