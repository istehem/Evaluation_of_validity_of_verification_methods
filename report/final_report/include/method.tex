...
Just passing a number of tests did not say much more then some tests evaluated
to true. The next thing to do was to evaluate what was actually tested.
...
The next step was to tweak the generators, that were used by
QuickCheck to construct valid API-calls. The tweaking of generators is
done to find better test cases. I.e. there were several branches
in the C-code that needed a specific sequence of API calls with
correct arguments, to be reached. For example, it is unreasonable to
test functions as often as half of the times when the initialization
function \lstinline!WdgM_Init! has not yet been called.

Because of QuickCheck's weight feature, one can simply change the
ratio of the generated API calls, by matching on the state and the
function you want to change probability to.

For example we have the initialization function \lstinline!WdgM_Init!
were we want to have a high priority if it has not been called yet,
and a low priority if it has been called previously.
\begin{lstlisting}
weight(S,  'WdgM_Init') ->
...
global status stayed in a good state more often; it was easier to do
positive testing.
...
specific configuration, see results.
...
\emph{eqc\_statem:show\_states/1}, see appendix \ref{APP:QUI/CKCHECK}. The
