...
the design choose in the C-code. This was done
...
be able to run further tests and test other parts of the code as
well. Some alternatives where discussed.
...
easy to see that the result was not good enough.

The next step was to tweak the generators used by Quickcheck to construct
API-calls. There was several branches in the C-code that needed a specific
sequence of API calls with correct arguments to be reached. Also the tweaking of
the generators could be implemented in a iterative way. Change the probability 
properties of the generators and check the results. The results were analyzed
and the generators were tweaked even more to make the results even better.

Used checking code coverage was however not good enough for our goals ... 
