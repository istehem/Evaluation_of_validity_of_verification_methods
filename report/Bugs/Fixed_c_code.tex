...
\begin{description}
  ...
  \item[7.2.3.2] \parbox[t]{0.8\linewidth}{Each SE may have M+N results from
      ...
      be overwritten}
  \item[WDGM255]\parbox[t]{0.8\linewidth}{Function Wdgm\_Init shall not change the
global status if a null pointer is passed as an argument.}
  \item[fig.4]\parbox[t]{0.8\linewidth}{The global status should be set to
   WDGM\_GLOBAL\_STATUS\_DEACTIVATED not WDGM\_GLOBAL\_STATUS\_OK from the very
beginning. That is before any function in WdgM.c is run.}
\end{description}
...
(//.Switch\_currentMode((Rte\_ModeType\_WdgMMode)status);)}
  ...
  \item[WdgM.h:39] WdgM\_PbCfg.h -> WdgM\_Pbcfg.h
\end{description}
