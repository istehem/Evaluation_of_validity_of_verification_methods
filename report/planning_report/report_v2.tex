...
%%This can be done by looking at the algorithms state space and argue for that
%%a formal verification tool checks that the states and
%every transitions between states are correct.
...
functional safety and reach an ASIL classification.
...
but that is just a statement for that everything has been executed. Have it been executed correctly?
One can also ask oneself, are every combination of computations in the system
necessary to ensure correctness? In other words is it possible to
collapse some states in the systems state space without endangering the safety
of the hole system?
...
exhaustively search all states due to the state explosion problem.
