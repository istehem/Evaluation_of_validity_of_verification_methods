...
deceases, with requirements that is confusing and contradictorily. The first
...
according to the ISO-standard, means.

The second problem is to evaluate if QuickCheck can be used to achieve this.

A model for an AUTOSAR module needs to be implemented.
How can one achieve good test cases for this model? How can one tweak the test
generation? More specific; which test cases are interesting in a
safety critical point of view? Is the implemented model good enough? Is it possible to
collapse states that do not affect the safety of the system?

An evaluation of the solution is needed, does it ensure functional safety? How
can one measure the size of the state space that are actually verified?
