...
verification means to prove the correctness of algorithms. This can be done by
arguing that the state space that is not checked will not affect the model.
In general, complex computer systems suffers from the state space explosion problem and
therefore exhaustively visiting all states are not practically possible.
...
functional safety according to the ISO-standard and achieve some form of ASIL
classification.
...
tool exist in the corporation where this thesis is done. %% Företaget hm....

A model for an AUTOSAR module needs to be implemented. For this to be a good
model some questions first need to be answered.
How can one achieve good test cases for this model? How can one tweak the test
generation to find test cases are interesting in a safety critical point of
view? Is the implemented model good enough? Is it possible to collapse
states that do not affect the safety of the system?
The test generation is a big problem then verifying a model. With unit tests, one
can argue that each line of code has been executed (100\% code coverage),
but that is just a statement of the execution. Have it been executed correctly?
...
for how certain the verification is. The confidence interval would help describe
the visited state space because it is probably not feasible to exhaustively search
all states.
